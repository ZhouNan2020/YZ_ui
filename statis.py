...
class MyApp:
    ...
    def tab1():
        if request.method == 'POST':
            ...
            if file and allowed_file(file.filename):
                ...
                self.sheetdict = pd.ExcelFile(os.path.join(app.config['UPLOAD_FOLDER'], filename)).parse(sheet_name=None) #使用pd.ExcelFile和parse方法读取文件中的所有sheet
                self.sheet_names = list(self.sheetdict.keys()) #直接从self.sheetdict中读取不同的键
                sheet_selected = request.form.get('sheet_selected') #获取选择的sheet
                if sheet_selected in self.sheetdict.keys(): #如果选择的sheet已经在self.sheetdict中存在
                    return render_template('tab1.html', sheetdict=self.sheetdict, sheet_selected=sheet_selected) #直接在页面上呈现对应的dataframe
            else:
                flash('请上传正确的文件格式')
        return render_template('tab1.html')

    @app.route('/tab1/upload', methods=['POST'])
    def upload():
        file = request.files['file']
        if file and allowed_file(file.filename):
            filename = secure_filename(file.filename)
            ...
            self.sheetdict = pd.ExcelFile(os.path.join(app.config['UPLOAD_FOLDER'], filename)).parse(sheet_name=None) #使用pd.ExcelFile和parse方法读取文件中的所有sheet
            self.sheet_names = list(self.sheetdict.keys()) #直接从self.sheetdict中读取不同的键
            return render_template('tab1.html', sheetdict=self.sheetdict)
        else:
            flash('请上传正确的文件格式')
    ...
    def tab2():
        st.title("按索引筛选")
        st.write("请上传索引文件")
        index = st.file_uploader("选择一个文件", type=["xls", "xlsx"],key='tab3')
        if st.button("开始筛选"):
            if index is not None: #如果上传了索引文件
                index_df = pd.read_excel(index) #读取索引文件
                if "subject_id" in index_df.columns: #如果索引文件中有subject_id列
                    if self.file is not None: #如果上传了筛选文件
                        ...
                        if len(sheet_dict) > 0: #如果筛选后的数据字典不为空
                            ...
                            return send_file('filterdata.xlsx', as_attachment=True) #添加一个下载按钮，用于下载筛选后的数据
